...
export default function CartItem({ data }) {
  const { title, price, imageURL, qty = 1 } = data;
  ...
  return (
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="flex flex-col justify-center col-span-6">
        <p>{title}</p> <button onClick={() => dispatch(increment_qty(data))}>+</button> {qty} <button onClick={() => dispatch(decrement_qty(data))}>-</button>
      </div>
